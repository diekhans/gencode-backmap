...
from pycbio_local.tsv import TsvReader
...
class CategoryCounts(object):
    ...
    def countTsv(self, tsvFile, getRowKeys, typeMap=None, filterFunc=None, getColumnKey=None):
        ...
        for row in TsvReader(tsvFile, typeMap=typeMap):
            if (filterFunc is None) or filterFunc(row):
                self.count(getRowKeys(row),
                           (self.defaultColumnKey if getColumnKey is None else getColumnKey(row)))
    ...
    def _writeMatrixRow(self, fh, rowKey, rowData, columnKeys, rowFrequencies, columnFrequencies, inclTotals):
        if rowFrequencies:
    ...
    def writeMatrix(self, fh, rowFrequencies=False, columnFrequencies=False, inclTotals=False):
        ...
        for rowKey in sorted(self.counts.keys()):
            self._writeMatrixRow(fh, rowKey, self.counts[rowKey], columnKeys, rowFrequencies, columnFrequencies, inclTotals)
        if inclTotals and not rowFrequencies:
            rowKey = ["all" for col in self.rowKeyHeader]
            self._writeMatrixRow(fh, rowKey, self.columnTotals, columnKeys, rowFrequencies, columnFrequencies, inclTotals)
...
class CategoryStatsCombine(object):
    ...
    def add(self, label, tsvFile):
        self._addNewLabel(label)
        iRow = 0
        for row in TsvReader(tsvFile):
            if iRow == 0:
                self._newSetInit(row)
            self._addRow(label, row)
            iRow += 1

    def _writeHeader(self, fh):
    ...
    def _writeRow(self, fh, key):
        row = list(key)
        for label in self.allLabels:
            row.extend(self.stats[key].get(label, self._emptyCounts))
        fileOps.prRow(fh, row)

    def write(self, fh):
        self._writeHeader(fh)
        for key in sorted(self.allKeys):
            if key != self.totalsKey:
                self._writeRow(fh, key)
        if self.totalsKey in self.stats:
            self._writeRow(fh, self.totalsKey)
